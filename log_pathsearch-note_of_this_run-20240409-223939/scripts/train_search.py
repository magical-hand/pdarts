...
parser.add_argument('--lamda_L1', type=float, default=0.9, help='L1 Penalty coefficient')
...
def main():
    ...
    for sp in range(len(num_to_keep)):
        model = Network(args.init_channels + int(add_width[sp]), CIFAR_CLASSES, args.layers + int(add_layers[sp]), criterion, switches_normal=switches_normal, switches_reduce=switches_reduce, p=float(drop_rate[sp]),args=args)
...
def train(train_queue, valid_queue, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=True):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if train_arch:
            ...
            if args.used_L1:
                loss_a = criterion(logits, target_search)+args.lamda*torch.sum(torch.abs(model.module.arch_weights()))
            else:
        ...
        if step % args.report_freq == 0:
            # if args.used_L1:
                logging.info('TRAIN Step: %03d Objs: %e R1: %f R5: %f arch_paramater:%f', step, objs.avg, top1.avg, top5.avg,model.module.arch_weights().cpu().detach().numpy())
                logging.info('arch_paramater:{:.5f}'.format(model.module.arch_weights().values()))
            # else:
            #     logging.info('TRAIN Step: %03d Objs: %e R1: %f R5: %f arch_paramater:%f', step, objs.avg, top1.avg,
            #                  top5.avg, model.module.arch_parameters())
