...
class Network(nn.Module):
    ...
    def __init__(self, C, num_classes, layers, criterion, args, steps=4, multiplier=4, stem_multiplier=3, switches_normal=[], switches_reduce=[], p=0.0):
...
#            cell = Cell(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, switches)
    ...
    def forward(self, input):
        ...
        for i, cell in enumerate(self.cells):
            ...
            s0, s1 = s1, cell(s0, s1, self._arch_weights)
    ...
    def _initialize_alphas(self):
        ...
        self.alphas_normal = nn.Parameter(torch.FloatTensor(np.random.randn(k, num_ops)))
        self.alphas_reduce = nn.Parameter(torch.FloatTensor(np.random.randn(k, num_ops)))
